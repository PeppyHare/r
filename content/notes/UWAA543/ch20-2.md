...
The 0th moment gives the conservation of mass relation. The Knudsen number \\( Kn \\) is the mean free path between collisions 
...
{{< katex display >}}
\overline{Q} = \begin{bmatrix} \rho \\ \rho u \\ \rho v \\ \rho w \\ B_x \\ B_y \\ B_z \\ e \end{bmatrix}
{{< /katex >}}
...
\mu \left( \pdv{u}{y} + \pdv{v}{x} \right) \\
\mu \left( \pdv{u}{z} + \pdv{w}{x} \right) \\
