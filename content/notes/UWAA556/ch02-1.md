...
Putting that into our definition of \\( \vec D \\) we get
...
If we define our new \\( \hat \vec D' \\) as
{{< katex display >}}
\hat \vec D' = \left( \hat \epsilon_b - \frac{\hat \sigma}{i \omega} \right) \hat \vec E
{{< /katex >}}
then
{{< katex display >}}
...
Looking back at what we've actually done when transforming to this new \\( \vec D' \\) view,
