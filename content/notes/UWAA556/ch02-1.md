...
{{< katex display >}}
\vec P = \epsilon_0 \overline \chi \vec E \qquad \vec D = \overline \epsilon \vec E
{{< /katex >}}
...
{{< katex display >}}
\dv{w_s}{t} = \vec E \cdot \vec j_s = \eta _s j^2 = \sigma_s E^2
{{< /katex >}}
...
Assuming a conductivity given by \\( \hat{\vec j}_f = \hat{\sigma} \hat{ \vec E} \\) and substituting in
{{< katex display >}}
i \vec k \cdot ( \hat{\sigma} \hat{\vec E} - i \omega \hat{\epsilon}_b \hat{\vec E}) = 0
{{< /katex >}}
{{< katex display >}}
i \vec k \cdot (\hat{\sigma} - i \omega \hat{\epsilon}_b ) \hat{\vec E} = 0
{{< /katex >}}
{{< katex display >}}
- i \omega(i \vec k \cdot(\hat{\epsilon}_b - \frac{\hat{\sigma}}{i \omega}) \hat{\vec E}) = 0
{{< /katex >}}

If we define our new \\( \hat{\vec D} ^\prime \\) as
{{< katex display >}}
\hat{\vec D}^\prime = \left( \hat{\epsilon}_b - \frac{\hat{\sigma}}{i \omega} \right) \hat{\vec E}
{{< /katex >}}
then
{{< katex display >}}
...
{{< katex display >}}
\vec D = \hat{\epsilon}_b \hat{\vec E} \quad \rightarrow \quad \hat{\vec D}' = \hat{\epsilon} \hat{\vec E}
{{< /katex >}}
{{< katex display >}}
\hat{\epsilon} = \hat{\epsilon}_b + i \frac{\hat{\sigma}}{\omega}
{{< /katex >}}
...
- If the polarization is happening such that the imaginary part of \\( \hat{\epsilon}_b \\) is zero, that implies that \\( \text{Im}(\hat{\epsilon}) = \hat{\sigma} / \omega \\)
...
{{< katex display >}}
\dv{^2 \vec x}{t^2} + \gamma \dv{\vec x}{t} + \omega_p ^2 \vec x = \frac{q}{m} \vec E_x
{{< /katex >}}
...
{{< katex display >}}
(- i \omega)^2 \hat{\vec x} + (- i \omega \gamma) \hat{\vec x} + \omega_p ^2 \hat{\vec x} = \frac{q}{m} \hat{\vec E}_x
{{< /katex >}}
Solving for the displacement,
{{< katex display >}}
\rightarrow \hat{\vec x}(\omega_p ^2 - \omega^2 - i \omega \gamma) = \frac{q}{m} \hat{\vec E}_x
{{< /katex >}}
{{< katex display >}}
\hat{\vec x} = \frac{q}{m} \frac{1}{\omega_p ^2 - \omega^2 - i \omega \gamma} \hat{\vec E}_x
{{< /katex >}}
...
{{< katex display >}}
\hat{\epsilon} = \hat{\epsilon}_b + i \frac{\hat{\sigma}}{\omega \epsilon_0}
{{< /katex >}}
{{< katex display >}}
\hat{\epsilon}_b = 1 + \frac{\omega_p ^2}{\omega_p ^2 - \omega^2}
{{< /katex >}}
