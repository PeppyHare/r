...
> {{< katex display >}}
{{< /katex >}}
> **Theorem** Given regular boundary conditions on \\( u(x) \\) for \\( x \in [a, b] \\), the Sturm-Liouville equation has
> 
...
{{< katex display >}}
\langle u_n u_m \rangle = \int _{-L/2} ^{L/2} u_m (x) u_n (x) \dd x = 2 L \delta _{n m}
{{< /katex >}}
...
{{< katex display >}}
f(x) = \sum_{n = - \infty} ^{\infty} c_n e^{i k _n x} \qquad k_n = \frac{2 \pi}{L} n \qquad c_n = \frac{1}{L} \int_0 ^{L} f(x) e^{- i k_n x} \dd x
{{< /katex >}}
...
In a discrete distribution, \\( X \\) can take on finitely/countably many values.
